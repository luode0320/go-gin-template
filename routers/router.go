...
func SetupRouter() *gin.Engine {
	...
	api1 := router.Group("/api/v1.0")
	{
		...
		api1.POST("/users/create", validator.ValidateForm(new(model.User)), userCtrl1.CreateUser) // 添加验证json参数中间件
	}
	...
	api2 := router.Group("/api/v2.0")
	{
		...
		api2.POST("/users/create", validator.ValidateForm(new(model.User)), userCtrl2.CreateUser) // 添加验证json参数中间件
	}
